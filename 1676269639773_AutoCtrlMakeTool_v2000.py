...
def GUI():
	...
	# Create a new window
	Win = cmds.window('Shape_Create_UI', t='AutoCtrlMakeTool', wh=(400,400), sizeable=True, mnb=1,mxb=0)
	# Parent Layout
	...
	if not sel_check:
		...
		for item in joint_list:
			...
			if pc==1:
					...
					print "Parent Constrained"
			else:
				print "Not Parent Constrained"
			
			if sc==1:
				cmds.scaleConstraint(item + '_C', item , mo=True )
				print "Scale Constrained"
			else:
				print "Not Scale Constrained"
			# ====================================================================================
			# Color of controls:
			print 'Color: ', col
...
def Change_Shape(Radius , Color , *args):
	global selection_for_shape_change
	print selection_for_shape_change
	i=0
	...
	if cmds.checkBox(Jnt_Prvd, query=True, value=True):
		...
		if not sel_check:
			...
			if make_fk == 1:
				...
				if cmds.radioButton(gb_01, query=True, select=True):
					...
					print 'Placement_C placed in proper group heirarchy'
					...
					print 'Placement_C scaling and visibility locked'
					...
					print 'Normal Scaling locked and hidden'
					...
					print 'Visibility and Scaling of Main, Global and Placement Ctrl managed'
					...
					print 'Vis channel of all individual Ctrls locked and hidden'
					cmds.select(cl=True)
					print Global_Rename_List
		...
		for item in shapes_list:
			...
			if cmds.radioButton(sk_01, query=True, select=True):
				...
				print 'Skin Cluster Added'	
			else:
				print 'Skin Cluster not added'
		...
		if make_grp==1:
			...
			print Rename_without_Global_List
